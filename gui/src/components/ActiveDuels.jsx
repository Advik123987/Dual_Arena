...
import { fetchActiveDuels } from '../api.js';

export default function ActiveDuels({ onSpectate }) {
            ...
            <div key={duel.room_id} className="duel-card-live">
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', alignItems: 'center' }}>
                <strong style={{ fontSize: '1.1rem' }}>
                  {duel.solo ? `🤖 ${duel.player_a} (Solo)` : `⚔️ ${duel.player_a} vs ${duel.player_b}`}
                </strong>
                <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'center' }}>
                  <span style={{ color: 'var(--accent-cyan)', fontFamily: 'var(--font-mono)' }}>{formatTime(duel.remaining)}</span>
                  <button
                    className="btn-secondary"
                    style={{ padding: '0.2rem 0.6rem', fontSize: '0.78rem' }}
                    onClick={() => onSpectate && onSpectate(duel)}
                  >
                    👁️ Spectate
                  </button>
                </div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', fontSize: '0.85rem' }}>
                <span style={{ background: 'rgba(255,255,255,0.1)', padding: '2px 8px', borderRadius: '12px', textTransform: 'capitalize' }}>{duel.category}</span>
                ...
                {duel.spectators_count > 0 && (
                  <span style={{ color: 'var(--accent-purple)', fontSize: '0.8rem', marginLeft: 'auto' }}>👁️ {duel.spectators_count} watching</span>
                )}
              </div>
            </div>
          ))}
